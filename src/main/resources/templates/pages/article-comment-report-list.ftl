                        ...
                        <div class="table-scrollable">
                            <table class="table table-bordered table-hover">
                                ...
                                <tr id="${p.getId()}">
                                    ...
                                    <td>${p.getContent()}</td>
                                    <td>${p.getTime()}</td>
                                    <td>${p.getComment().getUid()}</td>
                                    <td>${p.getComment().getCommentInfo()}</td>
                                    <td>${p.getReportId()}</td>
                                    <td>
                                        <#if disposeState == 0>
                                            <a class="btn btn-danger btn-xs" onclick="reportDispose('${p.getComment().getId()}',${p.getDisposeState()},0,${p.getId()})"><i class="btn-label glyphicon glyphicon-ok"></i>
                                                不管</a>
                                            <a class="btn btn-danger btn-xs" onclick="reportDispose('${p.getComment().getId()}',${p.getDisposeState()},1,${p.getId()})"><i class="fa fa-times"></i>
                                                撤销</a>
                                        </#if>
                                        <#if disposeState == 1>
...
<script>
    ...
    $("#selectType").change(function(){
        ...
        location = "/oa/report/comment/reportlist?disposeState="+disposeState
    });

    //举报处理
    function reportDispose(commentId,disposeState,dispose,id){
        $.post(
            "/oa/report/comment/reportdispose",
                {
                    commentId:commentId,
